...
(define (lookup target environment)
  (cond
    ...
    ((equal? target(first(second environment)))
     (second(second environment)))
    ( #t (error 'lookup "Variable Name Not Found"))
    )
)
...
(define (eval-and expression environment)
  (and
   (bool-eval (first (cdr expression)) environment)
   (bool-eval (first (cdr (cdr expression))) environment))
)
...
);What does this above even mean if I can just do what I did below?
(define (iffi e1 e2)
  (if (equal? e1 e2) #t #f))
...
(define (get-variables expression)
  (cond
    ...
    ((not(list? expression)) '())
    (#t (remove-duplicates (append (get-variables (first expression)) (get-variables (rest expression)))))
  )
)

;end defunc - what is defunc?
...
(define (make_bindings name)
  (cond
    ((equal? name null) '())
    ((is-variable? name)(list (list (list name #t)) (list (list name #f))))
    )
)
...
(define (insert_binding binding environments)
  (cond
    ((equal? environments null) '())
    ((is-variable? (first environments)) (list (list (first binding) environments)))
    (#t (append (insert_binding binding (first environments)) (insert_binding binding (cdr environments))))
    )
)
...
(define (insert_multiple_bindings bindings environments)
  (cond
    ((equal? environments null) bindings)
    ((equal? bindings null) '())
    (#t
