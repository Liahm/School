...
(define (no-or-above-and? expr)
  (cond
   [ (is-constant? expr) #t ]
   [ (is-variable? expr) #t ]
   [ (is-not? expr) (no-or-above-and? (op1 expr)) ] ;Double Negative
   [ (is-or? expr) (and (no-and? (op1 expr))(no-and?(op2 expr))) ] ;Distributive for or
   [ (is-and? expr) (and (no-or-above-and? (op1 expr))
                        (no-or-above-and? (op2 expr))) ] ;Distributive for and
   )
)
